...
const columns: AdminTableColumn[] = [
  ...
  { key: 'prescripteurs_count', label: 'Prescripteurs', sortable: true },
  { key: 'jeunes_count', label: 'Jeunes', sortable: true },
  { key: 'created_at', label: 'Date creation', sortable: true, hiddenBelow: 'md' },
]
...
async function handleAdd() {
  ...
  adding.value = true
  try {
    await $fetch('/api/admin/structures', { method: 'POST', body: { name } })
...
async function handleEdit() {
  ...
  editing.value = true
  try {
    await $fetch('/api/admin/structures', { method: 'PATCH', body: { id: editId.value, name } })
    toast.success('Structure renommée')
  ...
  <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="rounded-2xl border border-prado-border p-6 w-full max-w-md mx-4 shadow-xl" style="background-color: var(--prado-surface)">
          ...
          <form @submit.prevent="handleAdd">
            ...
            <input
              ...
              class="w-full px-4 py-3 rounded-xl bg-prado-input-bg border border-prado-border text-prado-text text-sm focus:outline-none focus:border-[var(--prado-signature)]/50 transition-colors mb-4"
            />
    ...
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" @click.self="showEditModal = false">
        <div class="rounded-2xl border border-prado-border p-6 w-full max-w-md mx-4 shadow-xl" style="background-color: var(--prado-surface)">
          <h3 class="text-lg font-semibold text-prado-text mb-4">Renommer la structure</h3>
          <form @submit.prevent="handleEdit">
            <label class="text-sm text-prado-text-secondary mb-1.5 block">Nouveau nom *</label>
            <input
              v-model="editName"
              type="text"
              required
