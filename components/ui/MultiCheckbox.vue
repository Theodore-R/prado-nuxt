  ...
  <div class="flex flex-wrap gap-2">
    <button
      ...
      :class="selected.includes(opt.value)
        ? 'bg-prado-signature-muted border-prado-signature/30 text-prado-text'
    ...
    >
      <span
        ...
        :class="selected.includes(opt.value)
          ? 'bg-prado-signature border-prado-signature'
          : 'border-prado-border'"
      >
        <svg v-if="selected.includes(opt.value)" class="w-2.5 h-2.5 text-prado-signature-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
